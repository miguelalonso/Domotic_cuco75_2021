void setupConfig(){
    ...
    pinMode(D5, INPUT); // Botón manual subir/bajar
          ...
          digitalWrite(D6,LOW); // D6  pulso abre/cierra Rele
          digitalWrite(D7,LOW);  // D7  alimenta motor
  
    Serial.begin ( 9600 );
    Serial.println("Rele motorizado Cuco 75!");
    ...
    conf.Latitud         =40.41;
    conf.Longitud        =-3.731;
    strcpy(conf.DevName,"ReleEntrada");
    ...
    if (strlen(conf.ntpServerName) < 3) {
      strcpy(conf.ntpServerName,ntpServerName);
    }
