...
//            DPRINTLN("");
 ...
 void sendDataConf(){
    ...
    int mm;
      ...
      doc["IP_3"] =         conf.IP_3;
      doc["IP_4"] =         conf.IP_4;
      doc["ntpServerName"] =         conf.ntpServerName;
 ...
 */
    ...
    valor= server.arg("ntpServerName");
    valor.toCharArray(conf.ntpServerName, 50);
...
void savePH(){
    DPRINTLN("Recibido comando savePH abrir/cerrar Estore a objetivo");
    conf.PH_max = server.arg("PH_max").toFloat();
    conf.PH_min = server.arg("PH_min").toFloat();
    saveConfig();
    String success = "1";
...
void sendGPIO() {
  ...
  json += "\"alta\":\"" + String(Rele_alta) + "\",";
  json += "\"baja\":\"" + String(Rele_baja) + "\"}";
